#!/usr/bin/env python3
"""
Sube los artículos de biblioteca-belleza/ a la tabla articulos_belleza en Supabase.
Lee los todos.json directamente, genera embeddings con google-generativeai (API key)
y hace insert por lotes.

Uso:
    python subir_belleza_supabase.py
    python subir_belleza_supabase.py --reset   # Empieza desde cero
"""
...
TABLE          = "articulos_belleza"
EMBED_MODEL    = "text-embedding-004"
BATCH_SIZE     = 50
PROGRESS_FILE  = Path("subir_belleza_progreso.txt")
...
def main() -> None:
    ...
    if total == 0:
        print("Sin artículos. Revisa que biblioteca-belleza/ tenga todos.json")
        sys.exit(1)

    # Retomar desde progreso
    offset = 0
